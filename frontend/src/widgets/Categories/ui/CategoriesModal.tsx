...
export const CategoriesModal = ({ onClose }: Props) => {
		...
		<>
			<div className='absolute right-0 left-0 z-7 h-[80dvh] overflow-hidden rounded-b-4xl bg-white py-10'>
				<div className='container flex items-start gap-10'>
						...
						{isPending
							...
							: parentCategories?.map((cat, index) => (
									<li
										...
										key={cat.id}
									>
										<Link className='flex items-center justify-between w-full' href={`/c/${cat.slug}`}>
					...
					{childCategories?.children.map(category => (
